...
class TaskManager {
	...
	static deleteUser(sirname) {
		...
		User.findOne({sirName: sirname}, function (err, user) {
			...
			if (user) {
				...
				// PROBABLY A BETTER WAY TO REMOVE ALL THE TASKS, FIND THE ONES THAT MATCH THE ID
	...
	static showTasks() {
		mongoose.connect(conString);
		// curl -d '{"text" : "Your message here", "bot_id" : "6e77c54c5133e94e916ca975d6"}' https://api.groupme.com/v3/bots/post
		// curl -X POST -H "Content-Type: application/json" -d '{"name": "Canis", "text": "Show Tasks"}' http://localhost:5000/

		// WE WANT TO CHANGE THIS SO WE FIND ALL THE TASKS FOR A USER BY THE ID IN THE TASKS COLLECTION
		...
		User.find(function(err, users) {
			...
			if (users.length == 0) {
				...
				for (var i = 0; i < users.length; i++) {
					...
					// NEED TO MAKE ANOTHER CALL TO THE TASKS COLLECTION
					// MAYBE JUST USE THE SHOWTASKSFOR FUNCTION AND USE DO THAT FOR EACH USER
	...
	static showTasksFor(sirname) {
		...
		// CHANGE THIS TO FETCH THE TASKS BY ID IN THE TASK COLLECTION

		// curl -X POST -H "Content-Type: application/json" -d '{"name": "Canis", "text": "Show Tasks - Canis"}' http://localhost:5000/
