				...
				users.forEach((item, index, array) => {
				  this.showTasksHelper(item, message, (usrmessage) => {
				    ...
				    if (counter === array.length && message === "") {
				    	...
				    	// Send message
						//Bot.sendMessage(message);
						console.log("sending the other messages");
				    }
				  });
				});
			}
	...
	static showTasksHelper(user, message, callback) {
		...
		if (user) {
			Task.find({ userID: user.id }, null, {sort: {taskNumber: 1}}, (err, tasks) => {
				...
				if (tasks.length > 0) {
					var singlemsg = "Sir ".concat(user.sirName).concat(" Tasks: \n");
					message = message.concat(singlemsg);
					for (var i = 0; i < tasks.length; i++) {
						singlemsg = singlemsg.concat("    ").concat(tasks[i].taskNumber).concat(" - ").concat(tasks[i].task).concat("\n");
						message = message.concat("    ").concat(tasks[i].taskNumber).concat(" - ").concat(tasks[i].task).concat("\n");
					}
					Bot.sendMessage(singlemsg);
				}
				callback(message);
			});
		}
