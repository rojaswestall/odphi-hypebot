...
class Bot {
    ...
    static checkMessage(message) {
        ...
        const vagabundo = /Vagabundo/;

        if (senderName && canisRegex.test(senderName) && hypeMeRegex.test(messageText)) {
            return "lol fabian fuck u";
        }

        if (senderName && hypeMeRegex.test(messageText) && (guajiroRegex.test(senderName) || vagabundoRegex.test(senderName))) {
            var neoPhrases = Array(
                ...
                "Hype yourself.");
            return phrases[Math.floor(Math.random()*neoPhrases.length)];
        }
        ...
        // if(messageText && fabesRegex.test(messageText)) {
        //     return "Fuck fabian in the ass";
        // }
