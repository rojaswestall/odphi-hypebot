...
class TaskManager {
		...
		User.findOne({sirName: sirname}, (err, user) => {
			...
			Task.findOne({taskNumber: taskNum, userID: user}, (err, task) => {
				...
				if (task) {
					...
					console.log("Task ".concat(taskNum).concat(" was removed for Sir ").concat(sirname));
					Bot.sendMessage("Removed task number ".concat(taskNum).concat(" for Sir ").concat(sirname));
				}
				else {
			...
			user.tasks.forEach((item, index, array) => {
				...
				if (spliced) {
					// TODO: After spliced, need to change the taskNumber for all the tasks after to be one less
				...
				// length = 2, remove 1
				// Remove first, processed = 1, length = 1
				// Will not check the second index

				// THIS IS THE CASE WHENEVER THE LAST IS REMOVED
				// Lentgh = 2, remove 2
				// check first, processed = 1, length = 2
				// check second, processed = 2, length = 1
	...
	static findAndRemove(array, property, value) {
		var myarr = array;
		myarr.forEach((result, index) => {
			if(result[property] === value) {
				//Remove from array
				console.log("result: ", result, "\n index: ", index);
			    myarr.splice(index, 1);
			    console.log("inside: ", myarr);
			}    
		});
		console.log("during", myarr);
		return myarr;
	}
